...
public class CarAccidents5Queries {
	public static void main(String[] args) {
		...
		final Dataset<Row> ds_lethal_accidents3 = ds_corrected.filter(ds_corrected.col("TOTAL_K").gt(0));
		final Dataset<Row> q3 = ds_lethal_accidents3.withColumn("WEEK", weekofyear(to_date(ds_lethal_accidents3.col("DATE"), "MM/dd/yyyy")))
																 .withColumn("YEAR", year(to_date(ds_lethal_accidents3.col("DATE"), "MM/dd/yyyy")))
																 .groupBy("YEAR", "WEEK").agg(count("TOTAL_K").as("N. LETHAL ACCIDENTS"));

		q3.write().format("csv").save(filePath + testNumber + "_" + String.valueOf(test_id) + "/query3/");

		
		final Dataset<Row> ds_lethal_accidents4 = ds_corrected.filter(ds_corrected.col("TOTAL_K").gt(0));
