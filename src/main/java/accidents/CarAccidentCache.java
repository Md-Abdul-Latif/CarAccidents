...
public class CarAccidentCache {
    public static void main(String[] args) {
        LogUtils.setLogLevel();

        final String filePath = args.length == 0 ? "./files/" : args[0];

        final SparkSession spark = SparkSession
                .builder()
                .master("local[*]")
                .appName("Car Accidents in New Yourk (cashe version)")
                .getOrCreate();
...
//        ds.printSchema();
     ...
     *
        ...
        //  final Dataset<Row> ds_con_1_f = ds_con_1.withColumnRenamed("UNIQUE KEY", "U1").withColumnRenamed("TOTAL_K", "TOT_K1");
        //  final Dataset<Row> ds_con_2_f = ds_con_2.withColumnRenamed("UNIQUE KEY", "U2").withColumnRenamed("TOTAL_K", "TOT_K2");
      ...
      */
        ...
        final Dataset<Row> q3 = q3_with_lethal_column.groupBy("BOROUGH","CORRECT_YEAR2","WEEK")
                .agg(count("UNIQUE KEY").as("N. ACCIDENTS"), avg("LETHAL?").as("avg(Lethal)"))
